...
socket.on('atualizar_status', (data) => {
    ...
    statusUsuario.innerHTML = `
        <span style="font-weight: bold; color: blue;">Seu código:</span> ${data.session_id} |
        <span style="color: green;">Pessoas à sua frente:</span> ${data.pessoas_a_frente}
    `;
    totalUsuarios.innerHTML = `
        <span style="color: red;">Total de usuários logados:</span> ${data.total_usuarios}
    `;
...
socket.on('atualizar_timer', (data) => {
    ...
    usuarioAtual.textContent = data.usuario_atual;
    timer.innerHTML = `
